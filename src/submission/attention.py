...
def precompute_rotary_emb(dim, max_positions):
    ...
    # Create a tensor of shape (max_positions, dim/2) containing the values of t * theta_i for each position t and each dimension i.
    t = torch.arange(max_positions).unsqueeze(1)  # Shape: (max_positions, 1)
    theta = torch.pow(10000, -2 * torch.arange(dim // 2) / dim)  # Shape: (dim/2,)
    values = t * theta  # Shape: (max_positions, dim/2)

    # Then, compute the cos and sin values for each position and dimension, and store them in a tensor of shape (max_positions, dim/2, 2) where the last dimension contains the cos and sin values.
    cos_values = torch.cos(values)  # Shape: (max_positions, dim/2)
    sin_values = torch.sin(values)  # Shape: (max_positions, dim/2)
...
def apply_rotary_emb(x, rope_cache):
    ...
    # Get the shape of the input tensor x, which is (B, nh, T, hs) where B is the batch size, nh is the number of heads, T is the sequence length, and hs is the head dimension.
    B, nh, T, hs = x.shape

    # Truncate the precomputed values in rope_cache to match the sequence length T.
    rope_cache_truncated = rope_cache[:T, :hs // 2, :]  # Shape: (T, hs/2, 2)

    # Reshape the cos and sin values in rope_cache_truncated to be broadcastable with the input tensor x.
    rope_cache_truncated = rope_cache_truncated.unsqueeze(0).unsqueeze(0)  # Shape: (1, 1, T, hs/2, 2)

    # Apply the RoPE to the complex tensor by multiplying it with the complex exponential of the cos and sin values.
    rope_complex = torch.view_as_complex(rope_cache_truncated)  # Shape: (1, 1, T, hs/2). Last dimension is now complex, where the real part is cos and the imaginary part is sin.

    # Convert the input tensor x to a complex tensor by viewing it as real and imaginary parts.
    x_complex = torch.view_as_complex(x.view(B, nh, T, hs // 2, 2))  # Shape: (B, nh, T, hs/2). Last dimension is now complex, where the real part is the first half of the head dimension and the imaginary part is the second half of the head dimension.

    # Multiply the input tensor x with the complex exponential of the cos and sin values to apply the RoPE. 
    rotated_x_complex = x_complex * rope_complex  # Shape: (B, nh, T, hs/2)

    # Convert the rotated complex tensor back to a real tensor by viewing it as real and imaginary parts.
    rotated_x = torch.view_as_real(rotated_x_complex) # Shape: (B, nh, T, hs/2, 2). Last dimension is now real, where the first part is the real part and the second part is the imaginary part.
    rotated_x = rotated_x.view(B, nh, T, hs)  # Shape: (B, nh, T, hs). The head dimension is now the original head dimension, where the first half contains the real part and the second half contains the imaginary part.

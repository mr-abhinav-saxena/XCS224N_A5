...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### START CODE HERE
        # grab a document from the data
        document = self.data[idx]

        doc_len = len(document)
        if doc_len < 4:
            document = document + self.PAD_CHAR*(4-doc_len) # so that truncated document is never less than 4 characters long
            doc_len = 4

        truncated_document_len = int(random.uniform(4, self.max_context_size))
        # truncated document can't be longer than the original document, so we take the min of the two
        truncated_document_len = min(truncated_document_len, len(document))

        truncated_document = document[:truncated_document_len]

        masked_content_len = int(random.uniform(0, truncated_document_len * self.masking_percent * 2)) # so that average of 1/4 the length of the truncated document is masked, but it can be more or less than that
        prefix_len = random.randint(0, truncated_document_len - masked_content_len)
        suffix_len = truncated_document_len - masked_content_len - prefix_len
        ...
        corrupted_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        corrupted_string = corrupted_string + self.PAD_CHAR*(self.block_size - len(corrupted_string)) # pad the corrupted string to be of length block_size

        x = corrupted_string[:-1]
        y = corrupted_string[1:]

        # encode x and y as Long tensors using the vocabulary
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
